...
def update_test_record(csv_filename, test_id, time_taken, in_progress):
    ...
    try:
        with open(csv_filename, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            rows = list(reader)
        for row in rows:
            if int(row['testID']) == test_id:
                row['timeTaken'] = str(time_taken)
                row['inProgress'] = str(in_progress)
...
def start_test():
    ...
    # Save test results to a CSV file named using the testID.
    results_df = pd.DataFrame(test_results["results"])
    results_filename = os.path.join(app.config['UPLOAD_FOLDER'], f"{test_id}.csv")
    try:
